...
create-users-table:
	...
	@./bin/postgres-check
	@docker-compose exec -T db psql -U "${POSTGRES_USER}" "${POSTGRES_DB}" -c "CREATE TABLE IF NOT EXISTS users (id UUID PRIMARY KEY, first_name VARCHAR, last_name VARCHAR, email VARCHAR);"
