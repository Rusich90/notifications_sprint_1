...
Команда для отправки тестового письма (Письма можно посмотреть на сервере mailhog http://localhost:8025)
```
make send-email
```

Команда для отправки тестового сообщения в очередь (Работу очередей можно просмотреть на сервере ui rabbitmq http://localhost:15672 логин и пароль в енвах)
```
make queue-produce
```

Команда для запуска тестового консьюмера
```
make queue-consumer
```
